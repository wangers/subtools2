...
if (system(
  "scripts/kaldi_utils/utt2spk_to_spk2utt.pl $out_dir/utt2spk >$out_dir/spk2utt") != 0) {
  die "Error creating spk2utt file in directory $out_dir";
}
system("env LC_COLLATE=C sscripts/kaldi_utils/fix_data_dir.sh $out_dir");
if (system("env LC_COLLATE=C scripts/kaldi_utils/validate_data_dir.sh --no-text --no-feats $out_dir") != 0) {

...
"""
PyTorch implementation of the scaled adam. Usually use with Eden scheduler.
Reference:
...
@OPTIMIZERS_.register(name="scaled_adam")
...
def _test_scaled_adam(hidden_dim: int = 200):
    import timeit
    ...
    for iter in [1, 0]:
        ...
        logger.info(f"Iter={iter}, Time taken: {stop - start}")

        logger.info(f"last lr = {scheduler.get_last_lr()}")
        # logger.info("state dict = ", scheduler.state_dict())
        # logger.info("optim state_dict = ", optim.state_dict())
        logger.info(f"input_magnitudes = {input_magnitudes}")
        logger.info(f"output_magnitudes = {output_magnitudes}")

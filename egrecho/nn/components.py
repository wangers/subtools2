...
"""
Some basic custom components/layers.
"""
...
class SEBlock_2D(torch.nn.Module):
    ...
    def forward(self, inputs: torch.Tensor):
        """
        @inputs: a 4-dimensional tensor (a batch), including [B, C, F, T]
        """
...
class TDNNBlock(nn.Module):
    ...
    Args:
        ...
        nonlinearity (str, optional):
            Specifies the type of nonlinearity to apply after the convolution. Default is 'relu'.
...
class TdnnAffine(torch.nn.Module):
    ...
    def __init__(
        ...
        for index in range(0, len(context) - 1):
            if context[index] >= context[index + 1]:
                raise ValueError(
                    "Context tuple {} is invalid, such as the order.".format(context)
                )
        ...
        self.weight = torch.nn.Parameter(
            torch.randn(output_dim, input_dim // groups, *kernel_size)
        )

        # For jit compiling.
        # 2021-07-08 Leo
        self.bias = (
            torch.nn.Parameter(torch.randn(output_dim)) if self.bool_bias else None
        )
    ...
    def extra_repr(self):
        return (
            "{input_dim}, {output_dim}, context={context}, bias={bool_bias}, stride={stride}, "
            "pad={pad}, groups={groups}, norm_w={norm_w}, norm_f={norm_f}".format(
                **self.__dict__
            )
        )
...
class FTdnnBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.factor = TdnnAffine(
            input_dim, bottleneck_dim, context_factor1, pad=pad, bias=False
        )
        self.affine = TdnnAffine(
            bottleneck_dim, output_dim, context_factor2, pad=pad, bias=True
        )
        self.relu = torch.nn.ReLU(inplace=True)
        self.bn = torch.nn.BatchNorm1d(
            output_dim, momentum=0.1, affine=True, track_running_stats=True
        )
...
class _BaseActivationBatchNorm(torch.nn.Module):
    ...
    def add_relu_bn(self, output_dim=None, options: dict = {}):
        ...
        default_params = dict_union(default_params, options)
        ...
        if not default_params["bn-relu"]:
            ...
            if default_params["bn"]:
                if not default_params["ln_replace"]:
                    ...
                    self.batchnorm = LayerNorm(
                        output_dim,
                        dim=1,
                        eps=1e-5,
                        learnabel_affine=default_params["bn_params"]["affine"],
                    )
        else:
            # BN-ReLU
        ...
        # actually the passing `jit_compile` is invalid, bc problem.
        default_params["jit_compile"] = (
            torch.jit.is_scripting() or torch.jit.is_tracing()
        )
        if (
            default_params["special_init"]
            and self.affine is not None
            and not default_params["jit_compile"]
        ):
...
class ReluBatchNormTdnnLayer(_BaseActivationBatchNorm):
    ...
    def __init__(
        self, input_dim, output_dim, context=[0], affine_type="tdnn", **options
    ):
        super(ReluBatchNormTdnnLayer, self).__init__()

        affine_options = {"bias": True, "groups": 1, "norm_w": False, "norm_f": False}

        affine_options = {**affine_options, **options}
...
class MultiAffine(torch.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, num=1, split_input=True, bias=True):
        ...
        if not isinstance(num, int):
            raise TypeError(
                "Expected an integer num, but got {}.".format(type(num).__name__)
            )
        if num < 1:
        ...
        self.weight = torch.nn.Parameter(
            torch.randn(1, self.num, self.output_dim, self.num_feature_every_part)
        )
    ...
    def forward(self, inputs):
        ...
        x = inputs.reshape(
            inputs.shape[0], -1, self.num_feature_every_part, inputs.shape[2]
        )

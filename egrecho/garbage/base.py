...
class PipeLine(ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # predefined key for the `from_pretrained` case
        self._resolved_opt: ResolveModelResult = None
        ...
        **kwargs,
        ...
        Args:
            ...
            model_type (str):
                ...
                :function::`egrecho.utils.io.resolve_pretrained_model`.
            feature_config (str or dict):
                A extractor config dict/config_file must include a key `"feature_extractor_type"` to instantiate.
                If not specified, it will be resolved from :function::`egrecho.utils.io.resolve_pretrained_model`
            ...
            return_pipeline (bool):
                Return pipline instance or its kwargs passing to `__init__` as dict·
            \**kwargs:
        ...
        feature_config = alt_none(feature_config, resolved_opt.feats_config)
        if (
        ...
        extractor_type = cls.load_extractor_type(
            feature_config["feature_extractor_type"]
        )
        feature_extractor = extractor_type.from_dict(feature_config)
        ...
        model_class = cls.load_model_type(model_type)
        ...
        if return_pipeline:
            pipeline = cls(
                ...
                device=DeviceMode.FROM_MODEL,
                **kwargs,
            )
            pipeline.resolved_opt = resolved_opt
            return pipeline
        else:
            return {
                "model": model,
    ...
    @property
    def resolved_opt(self) -> ResolveModelResult:
        return self._resolved_opt

    @resolved_opt.setter
    def resolved_opt(self, opt: ResolveModelResult):
        self._resolved_opt = opt

    @classmethod
    def load_model_type(cls, module_path: str, base_model_type: Type = TopVirtualModel):
        return load_model_type(module_path, base_model_type)

    @classmethod
    def load_extractor_type(cls, module_path: str, base_model_type: Type = BaseFeature):
        return load_extractor_type(module_path, base_model_type)

...
class ResponseImpulse(SinglePerturb):
    ...
    def _load_rir(self, lengths, impulse_duration_cut: Optional[float] = None):
        batch_size = len(lengths)
        if not hasattr(self, "noise_sampler"):
            self.noise_sampler = NoiseSet.from_dict(self.db_conf)
        noise_sampler_cnt = batch_size
        ...
        for i in range(noise_sampler_cnt):
            ...
            # cut too long rir signal to save computation
            if noise.size(-1) > lengths[i] or impulse_duration_cut is not None:
                impulse_duration_cut = alt_none(impulse_duration_cut, self.max_rir_len)
                direct_index = noise.abs().argmax(axis=1)[0].item()
                noise = noise[
                    ..., direct_index : direct_index + int(rate * impulse_duration_cut)
                ]
            noises.append(noise)
    ...
    def apply(
        ...
        **kwargs,
        ...
        if torch.rand(1) >= self.sim_prob:
            ...
            rir_ = self._load_rir(rir_lens, impulse_duration_cut=0.5)
        else:
        ...
        samples = fftconvolve1d(samples, rir_.expand(-1, channel, -1), mode="same")

...
@lru_cache(maxsize=8)
def get_lhotse_feat(**feat_conf):
    try:
        return LhotseFeat.from_dict(feat_conf)
    except KeyError as e:
        if _LHOTSE_AVAILABLE:
...
Feats_EXAMPLE_DOCSTRING = r"""
        ...
        .. code-block::
            ...
            extractor = KaldiFeatureExtractor(mean_norm=False, feat_conf={})
...
Feats_EXAMPLE_DOCSTRING_CLS = Feats_EXAMPLE_DOCSTRING.replace(
    "\n        ",
    "\n    ",
)


@add_end_docstrings(Feats_EXAMPLE_DOCSTRING_CLS)
class KaldiFeatureExtractor(SequenceFeature):
    ...
    @add_end_docstrings(Feats_EXAMPLE_DOCSTRING)
    def __call__(
        self,
    ...
    def get_sampling_rate(
        cls, extractor: LhotseFeat, sampling_rate: Optional[int] = None
    ) -> int:
        candidate_fields = [
            "sampling_rate",
        ...
        if sampling_rate is None and infered_sampling_rate is None:
            raise ConfigurationException(
                "`sampling_rate` is not set, and cannot be inferred from the extractor."
            )
        if (
    ...
    def available_extractors(self) -> List[str]:
        k = list(FEATURE_EXTRACTORS.keys())
        if _LHOTSE_AVAILABLE:
            from lhotse.features.base import FEATURE_EXTRACTORS as EXT_EXTRACTORS

            k += list(EXT_EXTRACTORS.keys())
        return list(set(k))
...
def cmvn_utts(
    ...
    if (int(attention_mask is None) + int(lengths is None)) == 0:
        raise ValueError("Set both attention_mask and lengths is ilegal.")

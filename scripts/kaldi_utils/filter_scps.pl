...
if(@ARGV != 4) {
  die "Usage: scripts/kaldi_utils/filter_scps.pl [-f <field-to-filter-on>] <job-range-specifier> <filter-pattern> <input-scp> <output-scp-pattern>\n" .
       "e.g.:  scripts/kaldi_utils/filter_scps.pl  JOB=1:10 data/train/split10/JOB/spk2utt data/train/feats.scp data/train/split10/JOB/feats.scp\n" .
       "similar to scripts/kaldi_utils/filter_scp.pl, but it uses multiple filters and output multiple filtered files.\n".
       "The -f option specifies the field in <input-scp> that we filter on (default: 1)." .
       "See also: scripts/kaldi_utils/filter_scp.pl\n";
}

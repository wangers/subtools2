...
if $has_segments; then
  for in_dir in $*; do
    if [ ! -f $in_dir/segments ]; then
      ...
      scripts/kaldi_utils/data/get_segments_for_data.sh $in_dir
    else
      cat $in_dir/segments
    fi
...
scripts/kaldi_utils/utt2spk_to_spk2utt.pl <$dest/utt2spk >$dest/spk2utt
...
if ! $skip_fix ; then
  scripts/kaldi_utils/fix_data_dir.sh $dest || exit 1;
fi

...
from utils.callback import LhotseCallback, SavePeftModelCallback, patch_skip_batches
...
DESCRIPTION = "Lora finetune whisper."


class LoraTune(BaseCommand):
    @classmethod
    ...
    def __init__(
        ...
        logger.info("加载LoRA模块...", ranks=0)
        if args.resume_from_checkpoint:
            # 恢复训练时加载Lora参数
            logger.info("Loading adapters from checkpoint.", ranks=0)
            model = PeftModel.from_pretrained(
                model, args.resume_from_checkpoint, is_trainable=True
            )
        else:
            logger.info("adding LoRA modules...", ranks=0)
        ...
        adam1_beta = args.lion_beta1 if 'lion' in args.optim else 0.9
        ...
        trainer = CustomTrainer(
            ...
            callbacks=[SavePeftModelCallback, LhotseCallback],
        )
        ...
        autocast = torch.autocast("cuda") if args.use_8bit else nullcontext()
        with autocast:
            # 开始训练
...
if __name__ == "__main__":
    parser = LoraTune.get_dummy_parser()
    parser = LoraTune.setup_parser(parser)
    args = parser.parse_args()
    logger.info(
        f"Got parsed args: \n{parser.dump(args.clone(),skip_default=True)}", ranks=[0]
    )
    LoraTune.run_from_args(args, parser)

...
def _replace_UnionType_with_typing_Union(annotation):
    from egrecho.utils.patch.simple_parse_patch._utils import (
        builtin_types,
        is_dict,
        is_list,
        is_tuple,
...
def get_field_type_from_annotations(some_class: type, field_name: str) -> type:
    ...
    while (
        ...
        and frame.f_locals.get(some_class.__name__) is not some_class
    ):
        # stack.append(frame)
        frame = frame.f_back
    ...
    try:
        ...
        with _initvar_patcher():
            annotations_dict = get_type_hints(
                Temp_, globalns=global_ns, localns=local_ns
            )
        field_type = annotations_dict[field_name]
    except Exception:
        logger.warning(

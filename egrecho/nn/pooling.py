...
from .components import ReluBatchNormTdnnLayer, TdnnAffine

ASV_POOLINGS = Register("poolings")
...
class BasePooling(torch.nn.Module):
    name: str
...
# Pooling ✿
@ASV_POOLINGS.register(name="stp")
class StatisticsPooling(BasePooling):
    """An usual mean [+ stddev] poolling layer"""

    def __init__(
        self, input_dim: int, stddev=True, unbiased=False, eps=1.0e-10, **kargs
    ):
    ...
    def forward(
        self, inputs, lengths: torch.Tensor = torch.ones((0), dtype=torch.long)
    ):
        """
        ...
        if lengths.size(0) > 0:
            ...
            for i in range(inputs.shape[0]):
                ...
                if self.stddev:
                    ...
                    var = (
                        torch.sum(
                            (inputs[i, :, :act_len] - mean_i) ** 2, dim=1, keepdim=True
                        )
                        / act_counts
                    )
...
class FreeStatisticsPooling(BasePooling):
    """An usual mean [+ stddev] poolling layer"""

    name = "fstp"

    def __init__(self, stddev=True, unbiased=False, eps=1.0e-10, **kargs):
    ...
    def forward(self, inputs):
        ...
        inputs = inputs.reshape(
            inputs.shape[0], -1, inputs.shape[len(inputs.shape) - 1]
        )
...
@ASV_POOLINGS.register(name="ldep")
class LDEPooling(BasePooling):
    ...
    def __init__(self, input_dim, c_num=64, eps=1.0e-10, **kargs):
    ...
    def forward(self, inputs):
        ...
        w = self.softmax_for_w(
            -(self.s**2 + self.eps) * torch.sum(r**2, dim=2, keepdim=True)
        )
...
@ASV_POOLINGS.register(name="xiep")
class xivec_stdinit_softplus2_prec_pooling(BasePooling):
    name = "xiep"
    ...
    def __init__(
        ...
        **kargs,
        ...
        self.prior_mean = torch.nn.Parameter(
            torch.zeros(1, input_dim), requires_grad=train_mean
        )
        self.prior_logprec = torch.nn.Parameter(
            torch.zeros(1, input_dim), requires_grad=train_prec
        )
    ...
    def forward(self, inputs):
        ...
        # Log-precision estimator
        logprec = self.softplus2(
            self.lin2(self.lin1_relu_bn(feat))
...
# Attention-based
class AttentionAlphaComponent(BasePooling):
    ...
    def forward(self, inputs):
        ...
        if self.num_head > 1 and self.temperature:
            if self.fixed:
                ...
                t = 1 + self.t**2
            x = (
                self.last_affine(x).reshape(batch_size, self.num_head, -1, chunk_size)
                / t
            )
...
@ASV_POOLINGS.register(name="asp")
class AttentiveStatisticsPooling(BasePooling):
...
@ASV_POOLINGS.register(name="mhap")
class MultiHeadAttentionPooling(BasePooling):
    ...
    def forward(self, inputs):
        ...
        mean = torch.sum(
            after_mul.reshape(batch_size, -1, chunk_size), dim=2, keepdim=True
        )
        ...
        if self.stddev:
            if self.stddev_attention:
                ...
                var = (
                    torch.sum(
                        after_mul_2.reshape(batch_size, -1, chunk_size),
                        dim=2,
                        keepdim=True,
                    )
                    - mean**2
...
@ASV_POOLINGS.register(name="gmhap")
class GlobalMultiHeadAttentionPooling(BasePooling):
...
@ASV_POOLINGS.register(name="mgmhap")
class MultiResolutionMultiHeadAttentionPooling(BasePooling):
...
# (Leo 2022-11-17)
@ASV_POOLINGS.register(name="mqmhasp")
class MQMHASP(BasePooling):
    """
    Reference:
...
# (Leo 2022-11-17)
@ASV_POOLINGS.register(name="mqmhasp_linear")
class MQMHASP_Linear(BasePooling):

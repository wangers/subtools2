...
from .annotation_utils.get_field_annotations import evaluate_string_annotation
...
def decode_field(
    ...
    with warnings.catch_warnings(record=True, **_kwargs) as warning_messages:
        if is_dataclass_type(field_type) and drop_extra_fields is not None:
            ...
            decoded_value = decoding_function(
                raw_value, drop_extra_fields=drop_extra_fields
            )
        else:
    ...
    if isinstance(type_annotation, str):
        ...
        if len(matching_entries) == 1:
            ...
            return try_functions(
                *(decoding_fn for _, decoding_fn in matching_entries.items())
            )
        else:

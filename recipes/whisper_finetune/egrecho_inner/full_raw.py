...
DESCRIPTION = "Full finetune whisper [ Original repo dataformat ]"
...
class FullTune(BaseCommand):
    ...
    def setup_parser(cls, parser: CommonParser):
        ...
        add_arg(
            "per_device_train_batch_size", type=int, default=8, help="训练的batch size"
        )
    ...
    def __init__(
        ...
        logger.info(f"训练数据：{len(train_dataset)}, 测试数据：{len(test_dataset)}", ranks=0)
        # 数据padding器

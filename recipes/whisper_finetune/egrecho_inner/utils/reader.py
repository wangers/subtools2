...
from .binary import DatasetReader


# support close pad audio fix 30s: pad_audio_fix (Leo 202406)
class CustomDataset(Dataset):
    def __init__(
        ...
        Args:
            ...
            pad_audio_fix: 是否将语音padding到30s.
        """
        ...
        self.pad_audio_fix = pad_audio_fix
    ...
    def _load_timestamps_transcript(self, transcript: List[dict]):
        assert isinstance(
            transcript, list
        ), f"transcript应该为list, 当前为：{type(transcript)}"
        data = dict()
    ...
    def __getitem__(self, idx):
        try:
            ...
            audio_pad_fixs = {} if self.pad_audio_fix else {"padding": True}
            if len(transcript) > 0:
                # 加载带有时间戳的文本
                if self.timestamps:
                    ...
                    data["input_features"] = self.processor(
                        audio=sample, sampling_rate=self.sample_rate, **audio_pad_fixs
                    ).input_features
                else:
                    # 获取log-Mel特征和标签ID

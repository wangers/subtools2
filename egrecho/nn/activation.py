...
class DoubleSwishFunction(torch.autograd.Function):
    """
    Definition: ``double_swish(x) = x * torch.sigmoid(x-1)``
    , originally motivated by its close numerical similarity to swish(swish(x)), where swish(x) =  x * sigmoid(x).
...
# Wrapper ✿
def Nonlinearity(nonlinearity: str = "relu", inplace=True, negative_slope=0.01):
    """A factory of activations."""

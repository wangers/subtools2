...
prefix=libritts
outdir=data/${prefix}/simplecuts_tts
...
if [ $stage -le 2 ] && [ $endstage -ge 2 ]; then
    log "stage 2: Prepare simple cuts."
    if [ ! -e $outdir/.libritts.done ]; then
      python local/prepare_cuts_simple.py \
        --outdir $outdir \
        --srcdir $lhotse_egsdir \
        --prefix $prefix \
        --parts ${parts}

      touch $outdir/.libritts.done
    fi
fi

expegs=exp/egs/${prefix}_simple
...
if [ $stage -le 3 ] && [ $endstage -ge 3 ]; then
  ...
  if [ ! -e ${expegs}/.libritts.train.done ]; then
    ...
    # train
    lhotse combine \
      data/${prefix}/simplecuts_tts/libritts_cuts_train-clean-100.jsonl.gz \
      data/${prefix}/simplecuts_tts/libritts_cuts_train-clean-360.jsonl.gz \
      data/${prefix}/simplecuts_tts/libritts_cuts_train-other-500.jsonl.gz \
      ${expegs}/cuts_train.jsonl.gz

    # dev
    lhotse copy \
      data/${prefix}/simplecuts_tts/libritts_cuts_dev-clean.jsonl.gz \
      ${expegs}/cuts_dev.jsonl.gz
    lhotse copy \
      data/${prefix}/simplecuts_tts/libritts_cuts_dev-other.jsonl.gz \
      ${expegs}/cuts_dev-other.jsonl.gz

    # test
    lhotse copy \
      data/${prefix}/simplecuts_tts/libritts_cuts_test-clean.jsonl.gz \
      ${expegs}/cuts_test.jsonl.gz
    lhotse copy \
      data/${prefix}/simplecuts_tts/libritts_cuts_test-other.jsonl.gz \
